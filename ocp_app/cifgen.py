...
def _get_mp_client(api_key: Optional[str] = None):
    """
    Materials Project client 생성.
    - mp-api 가 있으면 우선 사용
    - 아니면 pymatgen.ext.matproj.MPRester 사용
    """
...
def _standardize_to_conventional(
    ...
    *,
    ...
    """
    'MP 웹에서 Download CIF' 느낌에 최대한 가깝게 맞추기 위한 표준화:
      1) refined_structure
...
def _mpr_get_structure_by_material_id(
    ...
    *,
    ...
    """
    mp-api / legacy MPRester 양쪽을 최대한 커버.
    """
...
class DopingSpec:
    """
    (Deprecated) 기존 호환용. UI는 RatioTuneSpec 사용 권장.
    """
...
class RatioTuneSpec:
    """
    XY repeat 자동 탐색 + top-layer 금속 비율 튜닝(치환).

    추가(폭증 방지/직사각형 억제):
      - max_atoms_after_repeat: repeat 후 총 원자수 상한
      - prefer_square_xy: nx≈ny 및 aspect≈1 선호
    """
    target_ratio: Dict[str, int]
    layers_from_top: int = 1
    layer_tol: float = 0.3
...
def scale_xy_and_tune_ratio(atoms: Atoms, spec: RatioTuneSpec) -> Tuple[Atoms, Dict[str, Any]]:
    ...
    if spec.auto_scale_xy and base_sites > 0:
        ...
        # exact 요구했는데 못 찾으면 approximate fallback
