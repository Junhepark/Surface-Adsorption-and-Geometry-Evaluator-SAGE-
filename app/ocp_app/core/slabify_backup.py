...
from ocp_app.core.oxide_surface_rules import _pick_best_oxide_slab_candidate, _classify_surface_exposure
...
def _facet_warning_for_family_miller(surface_family: str, miller) -> str:
    ...
    if fam == 'rocksalt_AO' and hkl == (1, 1, 1):
        return 'polar oxide facet'
    if fam == 'rutile_AO2' and hkl in {(1, 0, 0), (1, 0, 1)}:
        return 'non-reference rutile facet'
    if fam == 'perovskite_ABO3':
        return 'termination-dependent perovskite facet'
    if fam == 'spinel_AB2O4':
        return 'cation-distribution-dependent spinel facet'
    return ''
